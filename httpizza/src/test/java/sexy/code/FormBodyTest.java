...
public final class FormBodyTest {
    ...
    public void urlEncoding() throws Exception {
        ...
        body.writeTo(os);
        os.flush();
        assertEquals(expected, baos.toString(Util.UTF_8.name()));
    }
    ...
    private String formEncode(int codePoint) throws IOException {
        ...
        body.writeTo(os);
        os.flush();
        String encodedCodePoint = baos.toString(Util.UTF_8.name());
        // Skip "a=b" prefix and "c" suffix
        return encodedCodePoint.substring(3, encodedCodePoint.length() - 1);
    }
}
